...
CHAT_HANDLERS.set('nuke', async (bot, data, username, msg) => {
  if (!msg) {
    return;
  }
  if (!(await bot.checkPermission(username, Rank.MOD, null))) {
    bot.sendChatMsg(`${username} does not have permission to nuke. FeelsWeirdMan`);
    return;
  }

  const words = msg.split(' ');
  const nukePhrase = words.substring(2);

  const lengthSec = parseDuration(words.slice(0).join(' '), /** format= */ 'sec');
  if (isNaN(lengthSec) || lengthSec < 0) {
    bot.sendChatMsg('Failed to parse timeout length. Example (30s): $nuke 30s 1m mods smell');
    return;
  }
  const nukeSecs = parseDuration(words.slice(1).join(' '), /** format= */ 'sec');
  if (isNaN(nukeSecs) || nukeSecs < 0) {
    bot.sendChatMsg('Failed to parse nuke length. Example (1m): $nuke 30s 1m mods smell');
    return;
  }
  const lengthDesc = humanizeDuration(nukeSecs * 1000);
  bot.sendChatMsg(`Nuking target phrase for ${lengthDesc}...`);
  const nend = getCurrentUnixTimestamp() + nukeSecs;
  bot.nukedPhrases.push({
    key: nukePhrase,
    value: nend,
  });
  // bot.lengthSecs.push(lengthSec);
  for (const msg of bot.recMsg.toArray()) {
    bot.timeoutUserIfMessageIsNuked(msg, username, lengthSecs);
  }
  // const tend = getCurrentUnixTimestamp() + lengthSecs;
  // await bot.db.setUserTimeout(username, tend);
});

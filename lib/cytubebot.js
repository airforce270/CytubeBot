...
export class CytubeBot {
  ...
  handleChatMsg(data, isPm) {
    ...
    if (username === '[server]') {
      ...
      if (joinMsg === 'joined') {
        const aliases = msg.split(' ')[3].slice(0, -1);
        this.checkAliases(uName, aliases);
        return;
      } else {
        return;
      }
    ...
    for (let w = 0; w < words.length; w++) {
      if (username != 'MrDestructoidCyDJ' && words[w].startsWith('MrDestructoidCyDJ')) {
  ...
  /**
   * Makes sure old aliases are not deleted before updating new ones.
   *
   * @param {string} user User name.
   * @param {string} currentAliases Aliases to check against ones that are already in database.
   *
   */
  async checkAliases(user, currentAliases) {
    let currentAToCheck = currentAliases.split(',');
    const aliasesDB = await this.db.getUserAliases(user);
    if ((aliasesDB.aliases === null) || (aliasesDB.aliases[0] === '')) {
      await this.db.updateUserAliases(user, currentAliases);
      infoLog.log(`!~~~! First time updating aliases for [${user}]: ${currentAliases}`);
      // Wait for user to load page
      await sleep(500);
      this.sendPm(
          user,
          `guraBlush h~ hi, I noticed you're a new user here. ` +
              `This guide can help you get started: https://tinyurl.com/CyDJguideV2`);
    } else {
      const aliasDB = aliasesDB.aliases.split(',');
      currentAToCheck = currentAToCheck.concat(aliasDB);
      const allAliases =
          currentAToCheck.filter((item, index) => currentAToCheck.indexOf(item) === index);
      await this.db.updateUserAliases(user, allAliases);
      infoLog.log(`!~~~! Updating aliases for [${user}]: ${allAliases}`);
    }
  }
  ...
  async handleLogin(data) {
    ...
    const startupQuotes = [
      `PagMan DM FOR FUTA`,
      `"is that poop" -spintto`,
      `Pepega cheeto`,
      `PepegaReading 𝒥𝒶𝓂 𝒶 𝑀𝒶𝓃 𝑜𝒻 𝐹𝑜𝓇𝓉𝓊𝓃𝑒 , 𝒶𝓃𝒹 𝒥 𝓂𝓊𝓈𝓉 𝓈𝑒𝑒𝓀 𝓂𝓎 𝐹𝑜𝓇𝓉𝓊𝓃𝑒`,
      `we hate xqc`,
      `BAAAAAAAAAT WE LOOOOOVE XQC`,
      'this.sendChatMsg(`Now handling commands. Times restarted: ${await this.readTimes()}`);',
      `guraCope dreading server soon..`,
      `" mendoS firetruck this place" -HentaiQueen`,
      `hackerCD`,
      `Type: TriHard to win a 4House`,
      `guraOMEGA`,
      `$cookie`,
      `ItMePeachy now has 0 cookies`,
      `Command of the day: $rngban`,
      `/me vented amongElis`,
      `"does it work" -JohnRG123`,
      `Anyways, um... I bought a whole bunch of shungite rocks, do you know what shungite is? ` +
          ...
          `I'm talkin' shungite. Anyways, it's a two billion year-old like, rock stone that prot` +
          `ects against frequencies and unwanted frequencies that may be traveling in the air. ` +
          `That's my story, I bought a whole bunch of stuff. Put 'em around the la casa. ` +
          `Little pyramids, stuff like that.`,
    ];
    const index = Math.floor(Math.random() * startupQuotes.length);
    await sleep(200);
    if (startupQuotes[index].startsWith('/me')) {
      this.sendChatMsg(`${startupQuotes[index]}`);
    } else if (startupQuotes[index].length > 300) {
      this.sendChatMsg(`${startupQuotes[index].substring(0, 296)}`);
      await sleep(50);
      this.sendChatMsg(`${startupQuotes[index].substring(296, startupQuotes[index].length)}`);
    } else {
      this.sendChatMsg(`[aqua]MOTD:[/] ${startupQuotes[index]}`, true);
    }

...
export class CytubeBot {
  ...
  handleChatMsg(data, isPm) {
    ...
    this.recMsg = new CircularBuffer(20);
    this.nukedPhrases = [];
    this.recMsg.enq(msg);
    this.timeoutUserIfMessageIsNuked(msg, username);
    // if (msg.toLowerCase().includes(key.toLowerCase)) {
      // bot.sendChatMsg(`/mute ${username}`, true);
      // const end = getCurrentUnixTimestamp + lengthSec;
  ...
  watchForNukeExpirations() {
    const checkIntervalSeconds = 10;
    setInterval(async () => {
      // const nukes = Object.keys(bot.nukedPhrases);
      // const end = Object.values(bot.nukedPhrases);
      const curTime = getCurrentUnixTimestamp;
      for (const [key, value] of bot.nukedPhrases.entries()) {
        if (curTime < value) {
          return;
        }
        if (curTime > value) {
          delete bot.nukedPhrases[key];
        }
      }
    }, checkIntervalSeconds * 1000);
  }

  async timeoutUserIfMessageIsNuked(msg, username, lengthSec) {
    // if (bot.recMsg.toLowerCase().includes(Object.keys.bot.nukedPhrases.toLowerCase())) {
    for (const [key, value] of bot.nukedPhrases.entries()) {
      if (bot.recMsg.toLowerCase().includes(key.toLowerCase)) {
        bot.sendChatMsg(`/mute ${username}`, true);
        const end = getCurrentUnixTimestamp + lengthSec;
        await bot.db.setUserTimeout(username, end);
        value = value;
    ...
    // copied up to handleChatMsg, this is a copy thats still here below.
    if (msg.toLowerCase().includes(key.toLowerCase)) {
      bot.sendChatMsg(`/mute ${username}`, true);
      const end = getCurrentUnixTimestamp + lengthSec;
      await bot.db.setUserTimeout(username, end);

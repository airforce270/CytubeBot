...
/**
 *
 * @typedef {Object} userAliases
 * @property {string} name Name to update aliases for.
 * @property {string} aliases Users aliases.
 */
...
export class Database {
   ...
   *
   * @param {string} username The user whose aliases we are updating.
   * @param {string} aliases The users aliases.
   * @return {!Promise<void>} Promise indicating the change's completion.
   */
  async updateUserAliases(username, aliases) {
    const statement = this.db.prepare(
        'UPDATE users SET aliases = ? WHERE LOWER(uname) = ?', [aliases, username.toLowerCase()]);
    return dbRunStatement(statement);
  }

  /**
   * Gets a users aliases.
   * @param {string} user Username to get aliases for.
   * @return {!Promise<!Array<!userAliases>>} Users aliases.
   */
